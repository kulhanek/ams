...
void CNodeItem::AddPackageDir(const CFileName& path,const CSmallString& build)
{
    ...
    while( it != ie ){
        ...
        if( p_node == NULL ){
            p_node = CNodeItemPtr(new CNodeItem);
            p_node->Name = *it;
            if( p_parent == NULL ){
...
void CNodeItem::ScanSoftRepoTree(const CFileName& path,int level)
{
    ...
    while( dir_enum.FindFile(child_name) ) {
        ...
        if( CFileSystem::IsDirectory(path / child_name) == true ){
            CNodeItemPtr child_item = SubItems[string(child_name)];
            if( child_item == NULL ) {
                child_item = CNodeItemPtr(new CNodeItem);
...
void CNodeItem::PrintTree(std::ostream& vout,const CFileName& path,int level)
{
    if( level > 0 ){
    ...
    std::map<std::string,CNodeItemPtr>::iterator    it = SubItems.begin();
    std::map<std::string,CNodeItemPtr>::iterator    ie = SubItems.end();

    while( it != ie ){
        CNodeItemPtr p_child = it->second;
...
void CNodeItem::PrintMissing(std::ostream& vout,const CFileName& path)
{
    std::map<std::string,CNodeItemPtr>::iterator    it = SubItems.begin();
    std::map<std::string,CNodeItemPtr>::iterator    ie = SubItems.end();
...
void CNodeItem::PrintExisting(std::ostream& vout,const CFileName& path)
{
    std::map<std::string,CNodeItemPtr>::iterator    it = SubItems.begin();
    std::map<std::string,CNodeItemPtr>::iterator    ie = SubItems.end();
...
void CNodeItem::PrintOrphans(std::ostream& vout,const CFileName& path)
{
    std::map<std::string,CNodeItemPtr>::iterator    it = SubItems.begin();
    std::map<std::string,CNodeItemPtr>::iterator    ie = SubItems.end();
...
void CNodeItem::CountOrphanedChilds(void)
{
    std::map<std::string,CNodeItemPtr>::iterator    it = SubItems.begin();
    std::map<std::string,CNodeItemPtr>::iterator    ie = SubItems.end();
