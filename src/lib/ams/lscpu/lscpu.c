...
int lscpu_init(int verbose)
{
    ...
    if( verbose == 0 ) return EXIT_SUCCESS;
...
struct lscpu_desc* lscpu_get(int verbose)
{
    if( lscpu_init(verbose) != EXIT_SUCCESS ) return(NULL);
    return(desc);
}
